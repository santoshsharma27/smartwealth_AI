...
describe('End-to-End Integration Tests', () => {
  ...
  describe('Judge Flow: Landing → Demo Data → Dashboard → Chat → Report', () => {
    it('loads demo data from landing and navigates to dashboard with full data', async () => {
      ...
      globalThis.fetch = createFetchMock({
        ...
        'GET /api/sessions/demo-session-001/summary': { status: 200, body: MOCK_SUMMARY },
        'GET /api/sessions/demo-session-001/score': { status: 200, body: MOCK_SCORE },
        'GET /api/sessions/demo-session-001/recommendations': { status: 200, body: MOCK_RECOMMENDATIONS },
        'GET /api/sessions/demo-session-001/recurring': { status: 200, body: MOCK_RECURRING },
        'GET /api/sessions/demo-session-001/anomalies': { status: 200, body: MOCK_ANOMALIES },
      });

      renderApp('/');

      // Landing page renders with required elements
      expect(screen.getByRole('heading', { level: 1, name: /SmartWealth AI/i })).toBeInTheDocument();
      expect(screen.getByRole('button', { name: /Try Demo Data/i })).toBeInTheDocument();
      ...
      await waitFor(() => {
        ...
        expect(screen.getAllByText(/Very Good/i).length).toBeGreaterThanOrEqual(1);
      });

      // Verify AI recommendations are displayed
      await waitFor(() => {
        expect(screen.getByText(/AI Recommendations/i)).toBeInTheDocument();
        expect(screen.getByText(/Your savings rate is 35%/i)).toBeInTheDocument();
      });
    ...
    it('navigates to chat page and sends a message with successful response', async () => {
      ...
      globalThis.fetch = vi.fn(async (url: string | URL | Request, init?: RequestInit) => {
        ...
        if (urlStr.includes('/summary')) {
          return new Response(JSON.stringify(MOCK_SUMMARY), { status: 200, headers: { 'Content-Type': 'application/json' } });
        }
        if (urlStr.includes('/chat') && method === 'POST') {
          return new Response(JSON.stringify(MOCK_CHAT_RESPONSE), { status: 200, headers: { 'Content-Type': 'application/json' } });
        }
      ...
      expect(screen.getByText(/informational guidance only/i)).toBeInTheDocument();
    });
    ...
    it('generates and downloads a report from the report page', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByRole('heading', { level: 1, name: /Financial Report/i })).toBeInTheDocument();
      });
      ...
      expect(screen.getByText(/Upload documents or load demo data/i)).toBeInTheDocument();
    });
    ...
    it('shows demo data load failure error on landing page', async () => {
      ...
      globalThis.fetch = createFetchMock({
        'POST /api/sessions': { status: 500, body: { error: 'Internal server error' } },
      });
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(/Demo data could not be loaded/i)).toBeInTheDocument();
      });

      // Buttons remain interactive
      expect(screen.getByRole('button', { name: /Try Demo Data/i })).toBeEnabled();
      expect(screen.getByRole('button', { name: /Upload Documents/i })).toBeEnabled();
    });
  });
  ...
  describe('User Flow: Landing → Upload → Processing → Dashboard → Goals → Chat → Report', () => {
    it('navigates from landing to upload page via Upload Documents button', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByRole('heading', { level: 1, name: /Upload Documents/i })).toBeInTheDocument();
      });

      // Verify upload form elements
      expect(screen.getByLabelText(/Select files to upload/i)).toBeInTheDocument();
    ...
    it('uploads a valid PDF file and shows processing status', async () => {
      ...
      globalThis.fetch = createFetchMock({
        'POST /api/sessions/test-session-001/documents': { status: 202, body: MOCK_DOCUMENT_UPLOAD_RESPONSE },
      });
      ...
      await waitFor(() => {
        expect(screen.getByLabelText(/Select document type for salary_jan\.pdf/i)).toBeInTheDocument();
      });

      const select = screen.getByLabelText(/Select document type for salary_jan\.pdf/i);
      await user.selectOptions(select, 'salary_slip');

      // Verify upload button is enabled
      const uploadBtn = screen.getByRole('button', { name: /Upload selected documents/i });
      ...
      await waitFor(() => {
        expect(screen.getByText(/uploaded successfully/i) || screen.getByText(/processing/i)).toBeTruthy();
      });
    });
    ...
    it('creates a goal and sees feasibility assessment result', async () => {
      ...
      globalThis.fetch = createFetchMock({
        ...
        'POST /api/sessions/test-session-001/goals': { status: 201, body: MOCK_GOAL },
      });
    ...
    it('sends a chat message and receives a data-driven response', async () => {
      ...
      globalThis.fetch = createFetchMock({
        'GET /api/sessions/test-session-001/chat/history': { status: 200, body: [] },
        'GET /api/sessions/test-session-001/summary': { status: 200, body: MOCK_SUMMARY },
        'POST /api/sessions/test-session-001/chat': {
          status: 200,
          body: {
            id: 'chat-2',
            question: 'What are my biggest expenses?',
            answer: 'Your largest expense category is Rent at ₹25,000/month, followed by Food at ₹18,500/month.',
            timestamp: new Date().toISOString(),
            disclaimer: 'This is informational guidance only.',
          },
        },
      ...
      // Verify the response
      await waitFor(() => {
        expect(screen.getByText(/Your largest expense category is Rent/i)).toBeInTheDocument();
      });
    });
    ...
    it('shows empty state on dashboard when no data is uploaded', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText(/No financial data/i) || screen.getByText(/Upload Documents/i)).toBeTruthy();
      });
    });
  });
  ...
  describe('Error Recovery: invalid file upload → error → valid upload → success', () => {
    it('shows format error for CSV used as salary slip then succeeds with PDF', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByLabelText(/Select document type for statement\.csv/i)).toBeInTheDocument();
      });

      const typeSelect = screen.getByLabelText(/Select document type for statement\.csv/i);
      ...
      await user.click(screen.getByRole('button', { name: /Remove statement\.csv/i }));
      ...
      const validSelect = screen.getByLabelText(/Select document type for salary_jan\.pdf/i);
      ...
      const uploadBtnValid = screen.getByRole('button', { name: /Upload selected documents/i });
    ...
    it('shows size error for oversized file then recovers with small valid file', async () => {
      ...
      // Select document type
      await waitFor(() => {
        expect(screen.getByLabelText(/Select document type for big_statement\.pdf/i)).toBeInTheDocument();
      });

      const typeSelect = screen.getByLabelText(/Select document type for big_statement\.pdf/i);
      await user.selectOptions(typeSelect, 'bank_statement');

      // Verify size error
      await waitFor(() => {
        expect(screen.getByText(/File exceeds maximum size of 10 MB/i)).toBeInTheDocument();
      });

      // Remove oversized file
      await user.click(screen.getByRole('button', { name: /Remove big_statement\.pdf/i }));
      ...
      await waitFor(() => {
        expect(screen.getByLabelText(/Select document type for bank_jan\.pdf/i)).toBeInTheDocument();
      });

      const validSelect = screen.getByLabelText(/Select document type for bank_jan\.pdf/i);
      await user.selectOptions(validSelect, 'bank_statement');

      // No errors should be shown
      expect(screen.queryByText(/File exceeds maximum size/i)).not.toBeInTheDocument();
    ...
    it('handles upload API failure and shows error message to user', async () => {
      ...
      // Upload a valid file
      const validFile = new File(['%PDF-1.4 content'], 'salary_jan.pdf', { type: 'application/pdf' });
      const input = screen.getByLabelText(/Select files to upload/i);
      ...
      expect(screen.getByRole('button', { name: /Upload selected documents/i })).toBeInTheDocument();
    });
  });
