...
describe("UploadPage", () => {
  ...
  describe("Rendering", () => {
    ...
    it("renders accepted file type information", () => {
      ...
      expect(
        screen.getByText(
          /PDF for salary slips, PDF or CSV for bank statements/,
        ),
      ).toBeInTheDocument();
    });
  ...
  describe("Format Validation", () => {
    it("shows error for non-PDF salary slip", async () => {
      ...
      const select = screen.getByLabelText(
        /select document type for data\.csv/i,
      );
      await user.selectOptions(select, "salary_slip");

      expect(
        screen.getByText(
          /file format not supported\. please upload PDF for salary slips/i,
        ),
      ).toBeInTheDocument();
    });
    ...
    it("shows error for non-PDF/CSV bank statement", async () => {
      ...
      const select = screen.getByLabelText(
        /select document type for image\.png/i,
      );
      await user.selectOptions(select, "bank_statement");

      expect(
        screen.getByText(
          /file format not supported\. please upload PDF or CSV for bank statements/i,
        ),
      ).toBeInTheDocument();
    });
    ...
    it("accepts PDF for salary slip without error", async () => {
      ...
      const select = screen.getByLabelText(
        /select document type for salary\.pdf/i,
      );
      await user.selectOptions(select, "salary_slip");
  ...
  describe("Size Validation", () => {
    ...
    //   const select = screen.getByLabelText(
    //     /select document type for large\.pdf/i,
    //   );
    //   await user.selectOptions(select, "salary_slip");
    ...
    it("shows error for empty (zero-byte) file", async () => {
      ...
      const select = screen.getByLabelText(
        /select document type for empty\.pdf/i,
      );
      await user.selectOptions(select, "salary_slip");
